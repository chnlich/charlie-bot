...
def _fake_launcher_dir(tmp_path: Path) -> Path:
  """A PATH dir whose charlie-code advertises --session-dir like the installed one.

  The CLI runs its refusals in a subprocess, where no in-process monkeypatch
  reaches; CI and any host without charlie-code need this on PATH for every
  refusal the launcher check must not fire ahead of.
  """
  bin_dir = tmp_path / "fake-launcher-bin"
  bin_dir.mkdir(exist_ok=True)
  fake = bin_dir / "charlie-code"
  fake.write_text('#!/bin/sh\n[ "$1" = --help ] && echo "usage: charlie-code [--session-dir DIR]\n"\nexit 0\n',
                  encoding="utf-8")
  fake.chmod(0o755)
  return bin_dir
...
def test_cli_addition_to_a_live_holder_refuses_structured_and_untouched(
  ...
  try:
    proc = _run_cli(["--home", str(home), "--port", str(_free_port()),
                    ...
                    launcher_dir=_fake_launcher_dir(tmp_path))
