...
(every session's metadata.json plus the active sessions' live chat files) into a
...
# Worst capped-search corpus snapshot: every session's metadata.json plus the
# active sessions' live chat files — exactly what the search route reads.
# Live home read once for the copy, never written.
root = Path.home() / ".charliebot" / "sessions"
...
| 2026-09-06 | this PR | M71 capped search request median 8.72/8.68/9.06 ms → 5.97/5.43/5.61 ms, maxima 40.15/42.27/40.51 ms → 8.35/6.87/6.88 ms (three interleaved rounds of the collector, 200 rows / 207 KB body, shared snapshot of 1086 metas + 163.5 MB active live chat files, parsed-body digest 7c1d6f5b519a identical across all six arms; manager-level corroboration 7.45 → 2.89 ms; absent-needle shape re-measured 2.69 → 2.92 ms, within noise; 4732-passed suite) | the 200-row cap applied to the sorted name matches before any row work: 200 newer-or-equal matches always outrank a match below the cap line and a content hit can only displace rows from above, so the dropped matches are never copied, stamped, probed, or sorted (the live corpus matched 1034 of 1086 names per request); the route serves the shared cached references through a read-only search returning (rows, resolve_sidebar_state dict) rendered via FastJsonResponse with thinking_since and the derived fields overlaid, the M56 /status shape; M71 definition and healthy range introduced with this PR |
