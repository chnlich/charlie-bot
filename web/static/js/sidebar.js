...
function renderGroupedSessionList(sessions, filter) {
  ...
  for (const key of sortedKeys) {
    ...
    const groupActions = key ? `
      <button onclick="event.stopPropagation(); renameGroup('${escapeHtmlAttr(key)}')"
      ...
      <button onclick="event.stopPropagation(); deleteGroup('${escapeHtmlAttr(key)}')"
...
async function renameGroup(oldName) {
  ...
  await fetch('/api/sessions/groups/rename', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
...
async function deleteGroup(groupName) {
  ...
  await fetch('/api/sessions/groups/delete', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
