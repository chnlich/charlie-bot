...
| 2026-09-06 | this PR | M55 first view 0.3078/0.3009 s → 0.1862 s (paired verbatim-collector arms on the shared 1.5 MB snapshot pair; in-process annotate wall median 0.2301 s → 0.1480 s over 5 timed calls each); repeat view unchanged 0.0025 s vs 0.0027 s (the M55 memo path); annotate output byte-identical across the real pair both directions plus 20 synthetic shapes (no-head, body-only, fragment, bare-text, unclosed, identical, empty arms, EOF insertions), exception parity included; served-body digest delta across arms is the ?v= git cache-bust token only | annotate dropped its third full HTMLParser parse (the spliced page was re-parsed only to re-derive head/body anchors the new page's own parse already holds — leaf and class renders insert inside body only, so the anchors cannot shift) and its two per-character splices became offset-sorted slice merges (insertion-list order preserved); the end-of-source header fallback keeps its after-the-insertions phase |
