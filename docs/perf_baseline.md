...
| M81 chat math-walk, delimiter gate | M81 collector below | seconds of KaTeX auto-render walk per message-page re-render (the M60 corpus) and per streamed math-free draft replay; the walks the gate skips count 0 | page re-render median < 0.020 s + 0.020 s per delimiter-bearing body on the page (recalibrated from < 0.020 s: the 2026-09-09 line was set on the all-math-free corpus the 36.3 MB worst file then was — readings 0.87-1.45 ms over 40 gate-skipped bodies; the worst file is now the 1051.3 MB runaway-turn capture carrying two $-bearing bodies whose walks are the page's own math rendering, 29.98-32.65 ms at 2 walks — see the 2026-09-16 history row; the formula degrades to the old line on a math-free page, where the gate's watch survives verbatim, and the walked count the collector prints is the gate's regression watch); streamed replay walk median < 0.010 s | — (introduced with its first history row) |
...
M7 warm-gate changed round — the in-server shape behind the standing changed-round reading
under active turns: the collect whose row memo and proof gate are warm and whose db rows
moved since the last collect. The standing changed-round collector re-seeds a fresh process
per round — a seeded gate miss takes the full key scan — so the warm gate needs its own
harness: a scratch message-table corpus sized to the live db's row count (built once at the
live mix's shape, turn rows dropped and the cache reset on reuse; the live db is read once
for the count, never written), one warm-up collect, then per timed round one turn's writes
(ten in-place step-finish upserts plus twenty appended rows, each bumping time_updated above
the table's max like drizzle's `$onUpdate`), the full key scan counted — a fallback round
prints its count and trips the line through its own price; the quiet round (the db signature
touched, no row moved — the probe skip) rides after, and a cold replay closes with the rows
digest as the parity witness:
...
if not CORPUS.is_file():
    ...
    blob = json.dumps({"role": "assistant", "modelID": "oc-m", "providerID": "prov",
                       "time": {"created": 1700000001000},
                       "tokens": {"input": 100, "output": 20, "cache": {"read": 4, "write": 2}},
                       "pad": "y" * 120})
    quiet = json.dumps({"role": "user", "pad": "z" * 120})
    payload = [(f"seed-{i}", "sess", 1700000000000 + i, 1700000000000 + i,
                blob if i % 2 == 0 else quiet) for i in range(live_rows)]
...
for stale in SCRATCH.glob("*.opencode_rows.json"):
    stale.unlink()
corpus_rows = sqlite3.connect(CORPUS).execute("select count(*) from message").fetchone()[0]
...
def synthesize_turn(tag: str) -> None:
    # One active turn's write shape: ten in-place step-finish upserts plus twenty appended
    # rows, each write bumping time_updated above the table's max like drizzle's $onUpdate.
    con = sqlite3.connect(CORPUS)
    rec = json.dumps({"role": "assistant", "modelID": "oc-m", "providerID": "prov",
                      "time": {"created": 1700000001000},
    ...
    for i, (mid,) in enumerate(con.execute("select id from message limit 10").fetchall()):
        con.execute("update message set data = ?, time_updated = ? where id = ?", (rec, tu + i, mid))
    tu += 10
    for i in range(20):
        con.execute("insert into message (id, session_id, time_created, time_updated, data) "
                    "values (?, 'sess', ?, ?, ?)", (f"{tag}-{i}", tu + i, tu + i, rec))
    con.commit()
    con.close()
...
print(f"{corpus_rows} corpus rows (live db {live_rows}); warm-gate changed round median "
      f"{changed[2] * 1000:.1f} ms, max {changed[-1] * 1000:.1f} ms over 5; quiet round "
      f"(probe skip) median {quiet[2] * 1000:.1f} ms; full key scans {full_scans}; rows digest "
      f"{digest}, replay digest {replay_digest}, parity {digest == replay_digest}")
EOF
```
...
| --- | --- | --- | --- |
| 2026-09-23 | this PR | M7 token-usage changed round under active turns, the warm row gate's proof miss advanced from a max-keyed tail fetch instead of the whole-table key scan: the in-process gate stores max(time_updated) beside the (count, sum) proof pair, the miss fetches only rows written after that max (every insert and every time_updated bump carries the write's own wall-clock ms — drizzle `$onUpdate`), and the fetched rows' own before/after sums reconstruct the pair's expected movement, a (count, sum) residual mismatch — a cascade delete, a clock stepped backward, a backward write — falling back to the full key scan; the persisted entry's probe stays the two-field (count, sum) pair the restart seed gates on, the max is restart-local, and the seeded restart's gate-skip fast path is unchanged: live-corpus harness changed-round median 417.6/425.3/444.2 → 300.7/303.3/302.6 ms (−27.9 % to −31.9 %), every paired round faster (three interleaved rounds, main checkout before vs branch worktree after, each a fresh process seeded from the live document+sidecar over a fresh copy of the 5.7 GB / 221,612-row live db, 30 moved rows per round — ten in-place step-finish upserts plus twenty appends at the production bump shape, load 2.3-2.7 one-minute carrying the standing sweep's tail), rows digest cf540435cc53 identical across every arm and the cold replay parity True; the doc collector's own calibration on a fresh synthetic corpus sized to the live row count: 355.7 → 163.8 ms (−54.0 %), full key scans 5 → 0, parity True; component attribution, standalone on the live db: the before shape's probe (count,sum) 63.7 ms + full keys scan 183.0 ms + Python per-id diff ~90 ms vs the after shape's probe (count,sum,max) 69.6 ms + tail fetch 59.0 ms — the max aggregate rides the scan the probe already ran; the quiet round (probe skip) reads 64.2-64.7 → 69.4-73.4 ms on the live-corpus harness (the +6 ms max aggregate) and 30.9-32.6 → 36.8-38.0 ms on the doc collector's corpus; production corroboration: the live server log's /token-usage page loads read 636-1420 ms server-side across the hourly cron's loads (30 loads over 6.55 h) — the changed-round shape the standing collector's fresh process cannot see (its document copy was seconds stale at sweep time, reading 0.086 s median); 76-passed token-tally suite including 4 new gate tests (tail-fetch apply with the persisted probe kept two-field, delete fallback, backward-write fallback, delete+insert count-residual fallback), 5943-passed full suite + 9 skipped (the 3 pre-existing environmental failures aside — the 2 nice-runner pins plus the load-sensitive fork-parks test, verified failing on the main checkout's main branch identically), ruff and yapf clean; M7 warm-gate sub-reading, collector, and healthy range (changed-round median < max(0.050 s, rows × 0.0000013 s), quiet round < 0.10 s) introduced with this PR | the changed collect's largest slice re-read the whole message table's keys and diffed 221k of them in Python on every hourly page load while an opencode turn was active — a cost that grows linearly with the db (85k → 190k → 221k rows across its landing docs) — where the tail fetch reads only the rows that moved and the residual check keeps the full scan for every move the fetch cannot prove complete |
