...
checkout under test decides. Evidence points the same collector at the before
...
| 2026-09-07 | this PR | M54 paint-work median 142.9/143.8/144.1 ms → 114.2/108.4/106.7 ms, maxima 237.1/230.5/236.4 ms → 219.9/206.0/206.5 ms (three interleaved rounds of the collector's replay, 11.4 KB worst fence-bearing on-disk draft sha1 b155f860788f, 59 deltas at 40 ms virtual cadence, 12 paints, page-pinned marked + hljs 11.9.0 common build (36 languages), main checkout 0cc96919 before vs branch worktree after back-to-back, every paired round faster; final-frame parity true all arms; component attribution on the same replay: highlightAuto 240.3 ms of the 261.0 ms paint work over 18 calls — the growing blocks' partial content re-highlighted per paint, cache misses by design; M33 stubbed-hljs replay re-measured 0.428 s median and M60 repeat-page 0.01 ms, both at their standing readings — no regression; 4740-passed suite plus 6 new stream-tail tests) | the block still growing at the draft's end (the fence open at EOF — whose content changes again before it settles, so its (lang, code) key can never hit the M54 highlight cache) renders escaped-plain during a streaming paint instead of re-running highlight per paint: openFenceTail reads the tail off the fence scanner fixNestedFences already walks (the earliest fence still open at EOF — marked's unterminated block starts there and swallows the rest, inner fence lines included), usage.js sets streamPaintTailCode around its parse, and renderer.code serves a cache-missed tail escaped; the paint where the fence closes, and the committed render after the turn, highlight once and the cache serves every later paint, so the final frame stays byte-identical (parity true) and every completed block keeps today's bytes (cache-first ordering) |
