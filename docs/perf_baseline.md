...
| M61 session-metadata read after TTL expiry, idle-cold | M61 collector below | ms per listing/get_session over the live corpus with every cache entry aged past `_METADATA_CACHE_TTL` (archived entries never expire; the idle cost is the non-archived set's revalidation) | bare listing median < 2 ms; single get_session median < 0.1 ms; archived-page median < 0.003 s; all-sessions median < max(0.008 s, cached-metas × 0.000008 s) (the full listing's model-copy floor scales with the cached set — archived entries never leave the cache, so it only grows — the way the M72 walk scales with listed entries and the M78/M84/M101/M107 lines track bytes; calibrated 2026-09-17 on 4.8-5.5 µs per cached meta across the 1075→1241 growth, the formula's margin also carrying the cold sidebar-probe term the fresh-manager collector shape adds) | — (introduced with its first history row) |
...
| --- | --- | --- | --- |
| 2026-09-17 | this PR | M61 all-sessions and archived-page sub-readings priced, docs-only calibration, no code change: the collector prints four idle-cold readings but the definition priced only bare listing and single get_session, so a regression in the dashboard's two real listing shapes (the archived tab's page and the full listing) tripped nothing. Readings the lines are set from — all-sessions: 5.50-5.78 ms at 1075 cached metas (#810's after arm, 2026-09-05), then 5.95/6.81/6.46 ms at 1238/1237/1241 metas (this round's three sweeps 11:45-14:43, load 1.5-1.9 one-minute) — 4.8-5.5 µs per cached meta across the 1075→1241 growth; archived-page: 2.09-2.13 ms at 1075 metas (#810's after arm), 0.99-1.09 ms today; the priced sub-readings stand inside their lines (bare listing 0.03-0.05 ms, single get_session 0.042-0.055 ms) | all-sessions median < max(0.008 s, cached-metas × 0.000008 s): the shape copies and sorts every cached meta (the ~1199-row archived set dominates) and its per-meta cost held at 4.8-5.5 µs across the corpus growth, so the line tracks the set the same corpus-tracking form the M78/M84/M101/M107 bytes-lines use — the 8 µs figure is 1.5-1.7x over the measured band and its margin carries the fresh-manager shape's cold sidebar-probe term; archived-page median < 0.003 s, 1.4x over #810's after band and 2.8x over today's readings |
