...
async def create_session(
  ...
  if not caller.is_operator:
    ...
    raise HTTPException(
        status_code=403, detail="an agent may only create a task directly under its own open manager task")
...
async def get_effective_prompt(
  ...
  if option is None:
    ...
    if not cfg.backends.options:
      raise HTTPException(
          status_code=400, detail="session backend resolution requires a configured backends.options entry")
...
async def _require_task_meta(task_mgr: TaskTreeManager, session_id: str) -> SessionMetadata:
  ...
  if meta is None:
    raise HTTPException(status_code=404, detail=SESSION_NOT_FOUND_DETAIL)
  if meta.profile is None:
    raise HTTPException(status_code=400, detail=f"session {session_id} is not a task-tree node (no profile)")
  return meta
