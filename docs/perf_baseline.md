...
```bash
git -C /home/chaoli/workspace/charlie-bot fetch origin main
...
A dirty or diverged checkout fails the restore and the round reports; the restore never discards
a sibling's work — an uncommitted tree fails `switch`, and the branch keeps its commits.
