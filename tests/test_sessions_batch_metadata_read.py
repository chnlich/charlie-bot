...
def _write_metadata(
    mgr: SessionManager, meta: SessionMetadata, raw: str | None = None
) -> Path:
...
def _failed_events(logs: list[dict], session_id: str) -> list[dict]:
    return [
        entry
        for entry in logs
        if entry.get("event") == "session_load_failed"
...
@pytest.mark.asyncio
async def test_batch_isolates_model_validate_json_failures_per_session(
    tmp_path: Path,
) -> None:
    mgr = _make_session_mgr(tmp_path)
    good = SessionMetadata(name="good")
...
async def test_batch_install_uses_setdefault_for_cache_entry_added_during_read(
    ...
    installed_during_read = SessionMetadata(
        id=disk_meta.id, name="installed during read"
    )
...
@pytest.mark.asyncio
async def test_batch_output_matches_sequential_get_session_for_mixed_fixture(
    tmp_path: Path,
) -> None:
    mgr = _make_session_mgr(tmp_path)
    active = SessionMetadata(name="active")
