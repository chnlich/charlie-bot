...
- The `if False: yield {}` lines in `tests/test_chat_cancel.py`, `tests/test_master_cc_consumer.py`,
  ...
  delete.
- `model_config` (thirteen pydantic `BaseModel` classes: `ScheduledTaskConfig` and
  `CharlieBotConfig` in `src/core/config.py`, `SwitchEventRequest` in `src/api/diag.py`, and
  ...
  validation error. Nothing in the repo reads the name (whole-repo grep finds only the thirteen
