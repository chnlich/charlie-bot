...
async def test_two_same_spec_siblings_readback_binds_to_request_identity(
    ...
    first = await tree.create_task(
        request_id="delegate-sibling-1", task_parent_id=manager.id, profile="worker",
        task=TaskSpec(goal=description, task_type=TaskType.QUICK_EDIT),
        name="sibling-1", backend=None, caller="operator")
    second = await tree.create_task(
        request_id="delegate-sibling-2", task_parent_id=manager.id, profile="worker",
        task=TaskSpec(goal=description, task_type=TaskType.QUICK_EDIT),
        name="sibling-2", backend=None, caller="operator")
    ...
    readback = find_local_task_child(
        manager.id, description=description, task_type="quick-edit",
    ...
    derived_child = await tree.create_task(
        request_id=derived, task_parent_id=manager.id, profile="worker",
        task=TaskSpec(goal=description, task_type=TaskType.QUICK_EDIT),
        name="derived", backend=None, caller="operator")
    readback_derived = find_local_task_child(
        manager.id, description=description, task_type="quick-edit")
...
async def test_readback_returns_none_without_the_bound_child(
    ...
    await tree.create_task(
        request_id="delegate-other", task_parent_id=manager.id, profile="worker",
        task=TaskSpec(goal="a different spec entirely", task_type=TaskType.QUICK_EDIT),
        name="other", backend=None, caller="operator")
    assert find_local_task_child(
        manager.id, description=description, task_type="quick-edit",
        request_id="delegate-missing") is None
    ...
    assert find_local_task_child(
        legacy.id, description="whatever", task_type="quick-edit",
        request_id="delegate-x") is None
...
async def test_readback_run_id_is_the_operation_work_run(
    ...
    child = await tree.create_task(
        request_id="delegate-runcheck", task_parent_id=manager.id, profile="worker",
        task=TaskSpec(goal=description, task_type=TaskType.QUICK_EDIT),
        name="c", backend=None, caller="operator")
    ...
    await tree.runs.register_run(RunRecord(
        id=work_run_id, session_id=child.id, kind="work", repo_path="/repo"))
    await tree.runs.register_run(RunRecord(
        id=review_run_id, session_id=child.id, kind="review",
