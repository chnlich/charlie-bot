...
def test_loop_axis_renders_both_iteration_number_forms() -> None:
  ...
  prompt = build_worker_prompt(
      DESCRIPTION,
      cfg=CharlieBotConfig(charliebot_home=Path("/tmp/unused"), paths={"worktree_dir": "/tmp/worktrees"}),
      loop_dir=LOOP_DIR,
...
def test_memory_block_renders_into_prompt(tmp_path: Path) -> None:
  ...
  prompt = build_worker_prompt(
      DESCRIPTION, cfg=CharlieBotConfig(charliebot_home=charliebot_home, paths={"worktree_dir": "/tmp/worktrees"}))
