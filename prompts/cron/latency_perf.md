...
workflow needs a moment to register the checks after the push. Before merging, run the
`code-review` skill against the request with `--comment`, act on its findings on the same branch,
and report a skipped review, with its reason, in the summary. With the checks green, confirm the
