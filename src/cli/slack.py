"""CLI verb for a session's own Slack thread, callable from a Slack-summoned session.
...
answers, or null for a round no summon started). A refusal (no Slack thread,
blank text, Slack rejected the post) exits non-zero with a JSON error on
stderr and persists nothing. The session is derived from cwd per the usual CLI
convention; the reply-format contract is prompts/slack_reply_format.md.
"""
