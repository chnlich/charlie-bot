...
async def make_manager(tree: TaskTreeManager, name: str = "Manager"):
  return await tree.create_task(
      request_id=f"manager-{name}", task_parent_id=None, profile="manager",
      task=TaskSpec(goal="standup"), name=name, backend=None, caller="operator")
...
async def test_bound_master_admits_one_typed_input_and_dispatches(
  ...
  assert triggers[0].get("id") == "cron:wake-manager:2026-01-01T03:00:00+00:00"
...
async def test_legacy_session_binding_refuses_the_v2_path(
  ...
  legacy = await session_mgr.create_session(
      CreateSessionRequest(name="Legacy session"), backend=OPUS_BACKEND_ID)
...
def test_scheduled_config_carries_and_validates_binding(tmp_path: Path) -> None:
  ...
  # A bound task may declare project: it is a display-only grouping label.
  task = ScheduledTaskConfig(name="t", cron="0 3 * * *", session_id="abc", project="web",
                             prompt="wake", backend=OPUS_BACKEND_ID)
  assert task.session_id == "abc" and task.project == "web"
  # mode without a binding is a load error.
  with pytest.raises(ValueError, match="requires 'session_id'"):
    ScheduledTaskConfig(name="t", cron="0 3 * * *", mode="master", prompt="wake", backend=OPUS_BACKEND_ID)
  # And mode master + explicit binding requires a prompt source.
  with pytest.raises(ValueError, match="requires a prompt source"):
    ScheduledTaskConfig(
        name="t", cron="0 3 * * *", session_id="abc", mode="master", handler="backup",
...
async def test_config_loader_and_api_round_trip_binding(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
  ...
  manager = await tree.create_task(
      request_id="manager", task_parent_id=None, profile="manager",
      task=TaskSpec(goal="standup"), name="Manager", backend=None, caller="operator")
  cron_dir = cfg.charliebot_home / "config.d" / "cron.d"
  cron_dir.mkdir(parents=True, exist_ok=True)
  wake_md = cron_dir / "wake.md"
  wake_md.write_text("wake the manager\n", encoding="utf-8")
  ...
  assert task.prompt == "wake the manager\n"
