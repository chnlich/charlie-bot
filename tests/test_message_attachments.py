...
@pytest.mark.asyncio
async def test_execute_command_improve_dispatches_master_prompt_without_state_write(tmp_path) -> None:
  cfg = CharlieBotConfig(charliebot_home=tmp_path / "charliebot-home")
  meta = SessionMetadata(name="Slash Session")
  session_mgr = AsyncMock()
  request = SimpleNamespace(app=SimpleNamespace(state=SimpleNamespace()))
  req = SlashExecuteRequest(command="improve", args="3 optimize startup")
  ...
  assert json.loads(response.body) == {
      ...
      "goal": "optimize startup",
      "max_iterations": 3,
  }
  session_mgr.persist_and_broadcast.assert_awaited_once()
  persisted_event = session_mgr.persist_and_broadcast.await_args.args[1]
  assert persisted_event["content"] == "/improve 3 optimize startup"
  assert mock_run.call_count == 1
  prompt = mock_run.call_args.args[2]
  assert "optimize startup" in prompt
  assert "python -m src.cli.improve" in prompt
  assert "Per-loop state will be created under:" in prompt
  assert not (cfg.sessions_dir / meta.id / "loops").exists()
