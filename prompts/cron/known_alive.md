...
- `aigw_api_key`, `feishu_app_id`, `feishu_app_secret`, `feishu_refresh_token`,
  ...
  `public_base_url` — have no in-repo script consumer (`gemini_api_key` has no prose mention
  outside this ledger — the config template's design note covers it through the `gemini_/`
  prefix set; `aigw_api_key` surfaces only in its own field comment). The four
