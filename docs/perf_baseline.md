...
source) carries none of the three delimiter initials (`$`, `\(`, `\[` — characters marked never
synthesizes and escapeHtml never adds or removes), so math-free pages and paints render
byte-identically without the scan. The cost is client-side, invisible to every HTTP probe, so
the collector loads the checkout's real markdown-renderer.js/usage.js with the page's CDN-pinned
katex 0.16.21 build over a jsdom DOM (the browser walks its own DOM natively; jsdom only supplies
one; one-time scratch install `npm i --prefix /tmp jsdom@24`, resolved through `JSDOM_HOME`,
...
200 ms coalesced paint cadence. Evidence points the collector at the before and after checkouts
...
| --- | --- | --- | --- |
| 2026-09-09 | this PR | M81 page re-render wall 41.86/32.90/39.07 → 1.28/0.98/1.32 ms, −97 %, 40 → 0 walks (three interleaved rounds of the new collector, 40 math-free bodies / 57.6 KB / corpus sha1 7409bcd20e4c of the 36.3 MB worst live chat file, katex 0.16.21 over jsdom, main checkout before vs branch worktree after back-to-back at load 3.0-3.1, every paired round faster, page innerHTML parity true every round); streamed math-free draft (11.4 KB, sha1 b155f860788f — the M54 corpus) 13 paints: walk wall 4.60/6.17/5.61 → 0.00/0.00/0.00 ms, gate scans 0.05-0.08 ms (13 calls) after; 4876-passed suite plus the 6-case gate suite | the chat's KaTeX auto-render walk scanned every prose text node for the four delimiters on every message re-render (each session switch and page re-render) and on every coalesced streamed paint even when the message carries no math — the walk M33's replay and M60's repeat-page metric both stub away, so no standing number saw it; renderChatMath now skips the walk when the message's own source carries none of the three delimiter initials ($, \(, \[ — characters marked never synthesizes and escapeHtml never adds or removes, so the skip is byte-identical): the streamed paint passes the draft text, message renders fall to the existing `.prose-msg[data-raw]`, elements with neither (raw backend output) keep the unconditional walk, and a $-bearing draft keeps the full walk — the worst on-disk draft (100 KB, $ inside) is unchanged by design |
