...
buffered draft (tool rows at the render cap on pages and the committed message, at the preview
bound on stream deltas), so one big tool_result turns into megabytes on every page build and every
switch back to the session (the live log's worst bootstrap tail: 890 ms, session 4914c102's 9.92 MB
Bash output). The collector resolves the active session whose live chat file carries the largest single
