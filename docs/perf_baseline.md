...
| M68 worker-list marked changed-poll rebuild | M68 collector below | seconds per body rebuild after one writer mark, worst on-disk thread-metadata corpus; the unchanged poll and its conditional are M36's shapes | median < 0.003 s (recalibrated from < 0.005 s: the old line sat on the TestClient/httpx harness floor the 2026-09-15 repairs removed — the served path reads 1.9-2.0 ms across the repair round's loads 0.8, the cron-collision bias the M56 history documents; see that history row) | — (introduced with its first history row) |
...
| M84 backend stream-line parse, worst on-disk raw log | M84 collector below | seconds per full replay of the raw-log tail-follow loop and the stdout-stream NDJSON funnel over the worst on-disk raw agent log (scratch copy, live home read-only) | tail-follow median < max(0.060 s, bytes ÷ 200 MB/s); stdout-stream median < max(0.040 s, bytes ÷ 250 MB/s) (recalibrated from the absolute 0.060/0.040 s lines: the worst on-disk raw log is now a 1050.9 MB / 150-line master-run log whose orjson+translate replay floor measures 2874-3455 ms, 304-366 MB/s, so the line tracks the corpus's own floor; the 0.060/0.040 s max() floors keep the small-corpus watch verbatim — see the 2026-09-16 history row) | — (introduced with its first history row) |
...
| M95 worker-log newest-first scans, reviewer completion + failed improve iteration | M95 collector below | seconds per reviewer-completion worker-summary scan (early stop at the first answer); seconds per failed-iteration judgment pair (the quota scan and the summary sharing one newest-first pass), worst on-disk worker log | review median < 0.005 s; judgment-pair median < 0.012 s (recalibrated from < 0.050 s: the pair's newest-first walk now parses only candidate-typed lines and skips the non-candidate lines' join — the old line sat on the whole-log parse's ~10 MB single-line orjson floor the prefilter removed; see the 2026-09-15 history row) | — (introduced with its first history row) |
...
| --- | --- | --- | --- |
| 2026-09-16 | this PR | M7 restart-cold standing reading classified as corpus growth, not a product regression: wall 1.005-1.092 s, 21 rows, scanned 0.0 MB, rows digest ed5ee3801b44 over five fresh-process runs (verbatim collector, live cache document copied per run, load 1.47-3.1 one-minute across the readings) against the < 0.5 s line the 2026-09-15 landing set on the 5.8 MB corpus (readings 0.29-0.31 s); no-regression proof: interleaved fresh-process A/B against the pre-#1676 checkout 9f6e8526, back-to-back rounds main 1.092/1.022/0.961 vs pre 1.089/0.982/0.952 s at load 1.4-2.0 one-minute — every paired round within 4 %, so neither #1676's parse carry nor #1690's cache-store single-homing moved the restart path; component attribution (cProfile, the standing fresh-process shape): sidecar orjson parse 0.21 s (25.6 MB), the seeded per-id key diff over 175,227 rows 0.55 s, the memoized sessions walk 0.17 s, the sidecar rewrite 0.08 s; healthy range recalibrated to median < max(0.5 s, (document + sidecar bytes) ÷ 25 MB/s) with this PR — the reading sits 1.15-1.35x inside (31.7 MB ÷ 25 MB/s = 1.27 s); the collector now prints the document+sidecar bytes the formula reads | the corpus is the opencode db's message table (the 23.4 GB live db file) — every streamed turn adds rows, so the sidecar's rows map grows unboundedly and the seeded restart's floor grows with it; the follow-up code fix, once the M7 topic's one-day skip window from #1676 passes: persist the post-scan probe in the sidecar so the seeded restart takes the same aggregate-gate skip the warm steady state takes (today the seeded path always pays the 175k-row key diff — the weaker-proof trade the warm path's docstring already documents) |
