...
| 2026-09-02 | #597 | M19 median 0.1128 s → 0.0143 s, max 0.1560 s → 0.0213 s (collector payload shape verbatim, main checkout before vs final branch head after back-to-back at load 1.15/1.07/0.93; framed line stream identical, 32 lines) | chunk-search through two str.find passes instead of the terminator alternation regex (re engine ~0.087 s per 16 MB measured vs memchr speed), piecewise fragment accumulation instead of concatenating the accumulated remainder per chunk |
