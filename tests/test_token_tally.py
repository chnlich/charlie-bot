...
def _cold_reference(collect):
  """Run one collect on reset state, then restore the caller sequence's partial state."""
  saved = (dict(tt._source_partials), dict(tt._claude_key_counts), dict(tt._claude_key_records),
           dict(tt._claude_key_loc), {k: set(v) for k, v in tt._claude_key_holders.items()},
           dict(tt._claude_orphan), tt._aggregate_memo, tt._tally_memo)
  tt._reset_aggregate_memo()
  try:
    return collect()
  finally:
    (tt._source_partials, tt._claude_key_counts, tt._claude_key_records, tt._claude_key_loc,
     tt._claude_key_holders, tt._claude_orphan, tt._aggregate_memo, tt._tally_memo) = saved


def _tally_snapshot(tally):
  rows = sorted((r.source, r.model, r.calls, r.in_fresh, r.cache_write, r.cache_read, r.output, r.first, r.last,
...
def test_incremental_partials_match_a_fresh_fold(tmp_path: Path) -> None:
  ...
  a contributor dropping a replayed key (orphan transfer), the last copy dropping, a file
  deletion, an account relabel and a cacheless round."""
  ...
  def write(home: Path, session: str, records: list[dict]) -> None:
    ...
    with (d / f"{session}.jsonl").open("w") as fh:
      fh.writelines(json.dumps(r) + "\n" for r in records)

  def collect(label: str, homes: dict, with_cache: bool = True) -> None:
    args = {"claude_homes": homes, "codex_homes": {}, "opencode_db": db,
            "cache_path": cache if with_cache else None}
    inc = collect_token_usage(**args)
    ref = _cold_reference(lambda: collect_token_usage(
        **{**args, "cache_path": tmp_path / "ref-cache.json"}))
    assert _tally_snapshot(inc) == _tally_snapshot(ref), label

  def rec(rid: str, ts: str, input_: int) -> dict:
    return {"message": {"id": rid, "model": NAME,
                        "usage": {"input_tokens": input_, "cache_creation_input_tokens": 0,
