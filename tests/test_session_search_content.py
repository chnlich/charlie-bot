...
async def _session_with_chat_content(cfg, session_mgr: SessionManager, body: str, name: str = "search-target"):
...
async def test_content_search_hits_and_misses(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
  ...
  hit = await _session_with_chat_content(cfg, mgr, '{"type":"user","content":"the Purple Fox jumped"}\n')
  miss = await _session_with_chat_content(cfg, mgr, '{"type":"user","content":"nothing relevant"}\n')
...
async def test_content_search_matches_across_chunk_boundaries(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
  ...
  session = await _session_with_chat_content(cfg, mgr, "x" * 14 + needle + "y" * 30)
...
async def test_content_search_needle_longer_than_one_chunk(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
  ...
  session = await _session_with_chat_content(cfg, mgr, "z" * 3 + needle + "z" * 3)
