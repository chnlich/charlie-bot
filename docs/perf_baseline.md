...
| 2026-08-31 | #507 | M13 steady-state render 0.0544 s → 0.0000 s, max 0.0659 s → 0.0000 s; +10-event tail render 0.00021 s; cold parse unchanged 0.0795 s (6.7 MB largest events file, 2177 worker events; rendered list identical) | worker events renders memoized per file keyed on (dev, ino, size) with appended-tail incremental parses and an 8-file LRU; M13 definition and healthy range introduced with this PR |
