...
import pytest
from conftest import apply_config_overrides, backend_option
...
async def test_thread_detail_endpoint_serves_every_configured_backend_type(
    backend_type: str, option_fields: dict, tmp_path: Path) -> None:
  ...
  cfg = CharlieBotConfig(
      charliebot_home=tmp_path / "home",
      backends={
          "options": [backend_option(id="opt", label="Opt", type=backend_type, **option_fields)],
      },
  )
  thread_mgr = ThreadManager(cfg)
