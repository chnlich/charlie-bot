...
| 2026-09-01 | #576 | M27 steady-state tolerant read median 319.3 µs → 17.9 µs, max 355.6 µs → 39.6 µs (collector verbatim, main checkout before at load 1.28/1.17/1.15 vs branch head after at load 1.13/1.14/1.14; worst plans corpus 15.4 KB / 12 plans, live state read-only; projected payload identical) | per-file (mtime_ns, size) memo with a 32-path LRU for read_plans_tolerant, mirroring the M24 trigger-list memo; M27 definition and healthy range introduced with this PR |
